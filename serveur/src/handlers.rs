...
use crate::{GlobalEvent, log_event};
...
pub fn process_command(
    ...
    match command {
        GameCommand::Connect(pseudo) => (handle_connect(addr, pseudo, state, tx), false),
        ...
        GameCommand::Info(cible) => (handle_info(addr, cible, state, world), false),
        GameCommand::Equip(cible) => (handle_equip(addr, cible, state, world), false),
        GameCommand::Unequip(cible) => (handle_unequip(addr, cible, state, world), false),
        GameCommand::Equipment => (handle_equipment(addr, state, world), false),
        GameCommand::Take(cible) => (handle_take(addr, cible, state, world), false),
        GameCommand::Drop(cible) => (handle_drop(addr, cible, state, world), false),
        GameCommand::Talk(cible) => (handle_talk(addr, cible, state, world), false),
        GameCommand::Attack(cible) => (handle_attack(addr, cible, state, world, tx), false),
...
// ----------------------------------------------------------------------------
// FONCTIONS HANDLERS (Une par action)
// ----------------------------------------------------------------------------

fn handle_connect(addr: SocketAddr, pseudo: String, state: &mut ServerState, tx: &Sender<GlobalEvent>) -> String {
    ...
    if let Some(player) = state.players.get_mut(&addr) {
        ...
        for (slot, item_id) in &player.equipment {
            if let Some(item_def) = world.world.items.iter().find(|i| &i.id == item_id) {
                if slot == "weapon" { bonus_arme += item_def.damage.unwrap_or(0); } 
                else if slot == "head" || slot == "chest" || slot == "legs" { armure_joueur += item_def.defense.unwrap_or(0); }
            }
        }
    ...
    if let Some(m_id) = monstre_id_trouve {
        ...
        if let Some(hp) = state.npc_hps.get_mut(&m_id) {
            *hp -= degats_finaux;
            ...
            log_event("COMBAT", &joueur_nom, json!({"target": monstre_nom, "damage_dealt": degats_finaux, "target_hp_left": pv_monstre_restants}));
        ...
        if monstre_mort {
                ...
                for drop in drops_monstre {
                    if rand::thread_rng().gen_range(1..=100) <= drop.chance {
                        *player.inventory.entry(drop.item_id.clone()).or_insert(0) += 1;
                        let nom_item = world.world.items.iter().find(|i| i.id == drop.item_id).map(|i| i.name.clone()).unwrap_or(drop.item_id.clone());
            ...
            if !noms_drops.is_empty() { msg.push_str(&format!("Tu obtiens directement : {}\n", noms_drops.join(", "))); }
            msg
        } else {
    ...
    if let Some(player) = state.players.get_mut(&addr) {
        ...
        match channel.as_str() {
            "GLOBAL" => {
                ...
                log_event("CHAT", &username, json!({"channel": "GLOBAL", "message": message}));
            ...
            "ROOM" => {
                ...
                log_event("CHAT", &username, json!({"channel": "ROOM", "room": current_room, "message": message}));
                let _ = tx.send(GlobalEvent { sender_addr: addr, message: format_evt });
                "S: OK\n".to_string()
            }
            "GROUP" => {
                let format_evt = format!("S: EVT GROUP CHAT {} {}\n", username, message);
                log_event("CHAT", &username, json!({"channel": "GROUP", "message": message}));
                let _ = tx.send(GlobalEvent { sender_addr: addr, message: format_evt });
                "S: OK\n".to_string()
            }
